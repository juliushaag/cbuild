...
class Program:
  ...
  def run(self, args : str) -> tuple[Any, Any, int]:
    return self.run_async(args).wait()
  
  def run_async(self, args : str) -> "Process":
    cmd = [self.program] + (args.split(" ") if args is not None else [])
    link = subprocess.Popen(cmd, shell=True, stdout=subprocess.PIPE, stderr=subprocess.PIPE, env=os.environ.copy(), universal_newlines=True)

    return Process(self, link, cmd)
